...
class _RegisterViewState extends State<RegisterView> {
  ...
  void pick()async{
    ...
    if(pickedFile?.files.first.path != null){
      setState(() {
        profile = File(pickedFile!.files.first.path!);
      });
    }
  }
                  ...
                  SignUpForm(
                    ...
                    register: () async{
                      if(nameController.text.isNotEmpty && emailController.text.isNotEmpty && passwordController.text.isNotEmpty){
                        final response = await vm.register(nameController.text, emailController.text, passwordController.text, bioController.text, profile?.path);
                        AwesomeDialog(
                          context: context,
...
class SignUpForm extends StatelessWidget {
  const SignUpForm({required this.name, required this.email, required this.password, required this.bio, required this.register});
  final TextEditingController name, email, password, bio;
      ...
      child: Column(
        children: [
          ...
          ElevatedButton(
            ...
            child: vm.isLoading ? const Center(child: CircularProgressIndicator(),) : const Text("Continue"),
          )
        ],
      ),
    );
