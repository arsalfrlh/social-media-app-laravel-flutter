...
import 'package:sosmed/views/auth/register_view.dart';
                  ...
                  SignInForm(
                    ...
                    login: () async{
                      if(emailController.text.isNotEmpty && passwordController.text.isNotEmpty){
                        final response = await vm.login(emailController.text, passwordController.text);
                        AwesomeDialog(
                          context: context,
...
class SignInForm extends StatelessWidget {
  const SignInForm({required this.email, required this.password, required this.login});
    ...
    return Form(
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          ElevatedButton(
            onPressed: vm.isLoading ? null : login,
    ...
    return Row(
      ...
      children: [
        ...
        GestureDetector(
          onTap: () {
            Navigator.pushAndRemoveUntil(
              context, MaterialPageRoute(builder: (context) => RegisterView()), (route) => false);
          },
          child: const Text(
            "Sign Up",
            style: TextStyle(
